...
-- SQL TRACE -2 STARTED AT 2026-03-12 18:52:06.473089 --

-- Session: 51 | Start: 2026-03-12 18:53:22.623000 | Status: running | Cmd: SELECT
SELECT target_data
...
-- Session: 60 | Start: 2026-03-12 18:57:39.293000 | Status: runnable | Cmd: SELECT
...
-- Session: 59 | Start: 2026-03-12 18:59:34.900000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
...
-- Session: 69 | Start: 2026-03-12 19:01:34.443000 | Status: runnable | Cmd: SELECT
-- This script extracts inventory, costs, rotation, and expiration classification,
-- ensuring that the next expiration date (ProximaFechaV) is only taken from lots with active stock (Cantidad > 0).
...
-- Session: 64 | Start: 2026-03-12 19:04:40.197000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 64 | Start: 2026-03-12 19:05:01.503000 | Status: running | Cmd: SELECT
SELECT A.*
FROM SFTITM A
ORDER BY A.itemid ASC
GO

-- Session: 59 | Start: 2026-03-12 19:05:34.187000 | Status: runnable | Cmd: SELECT
-- This script extracts inventory, costs, rotation, and expiration classification,
-- ensuring that the next expiration date (ProximaFechaV) is only taken from lots with active stock (Cantidad > 0).
...
-- Session: 64 | Start: 2026-03-12 19:10:08.730000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT  * , ROW_NUMBER() OVER (ORDER BY Descrip ASC) AS ROWNUM   FROM SACLIE WITH (NOLOCK) 
  WHERE (ACTIVO>0))
...
-- Session: 64 | Start: 2026-03-12 19:11:00.517000 | Status: running | Cmd: SET COMMAND
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
  ...
  WHERE ((SP.CodProd LIKE 'ACICLOVIR%') OR (SP.DescripAll LIKE 'ACICLOVIR%') OR (SP.Refere LIKE 'ACICLOVIR%') OR (SP.Existen LIKE 'ACICLOVIR%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 35
GO

-- Session: 64 | Start: 2026-03-12 19:11:03.130000 | Status: running | Cmd: SELECT
...
-- Session: 61 | Start: 2026-03-12 19:11:14.037000 | Status: runnable | Cmd: SELECT
-- This script extracts inventory, costs, rotation, and expiration classification,
-- ensuring that the next expiration date (ProximaFechaV) is only taken from lots with active stock (Cantidad > 0).
...
-- Session: 58 | Start: 2026-03-12 19:17:43.350000 | Status: running | Cmd: SELECT
-- This script extracts inventory, costs, rotation, and expiration classification,
-- ensuring that the next expiration date (ProximaFechaV) is only taken from lots with active stock (Cantidad > 0).
...
-- Session: 51 | Start: 2026-03-12 19:18:29.437000 | Status: suspended | Cmd: DROP EVENT SESSION
...
-- Session: 64 | Start: 2026-03-12 19:21:32.277000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT lo.nrounico, lo.nrolote, lo.codubic, dp.descrip, lo.costo, Precio3 AS PRECIO, dbo.FN_ADM_TAXPRODUCT(CodProd,Precio3,1,0,0)+Precio3 As PrecioTx,PrecioU3 AS PRECIOU, dbo.FN_ADM_TAXPRODUCT(CodProd,PrecioU3,1,1,0)+PrecioU3 As PrecioUTx,lo.cantidad, lo.cantidadu, lo.fechae, lo.fechav from SALOTE lo Inner Join sadepo dp on lo.codubic=dp.codubic where (lo.codprod='7591585219299') And (lo.CodUbic='AMR001')  And ((lo.Cantidad>0) Or (lo.CantidadU>0)) Order By lo.codubic, lo.fechav
GO

-- Session: 67 | Start: 2026-03-12 19:22:04.700000 | Status: running | Cmd: SELECT
-- This script extracts inventory, costs, rotation, and expiration classification,
-- ensuring that the next expiration date (ProximaFechaV) is only taken from lots with active stock (Cantidad > 0).
...
-- Session: 58 | Start: 2026-03-12 19:33:43.710000 | Status: running | Cmd: SELECT
-- Query for 'Lotes' worksheet: filters lots based on entry date, rotation and quantity.
SELECT
    SALOTE.CodProd AS Cod,
    SALOTE.NroLote,
...
-- Session: 64 | Start: 2026-03-12 19:34:05.280000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='ACETAMI' OR P.CodProd='ACETAMI')
GO

-- Session: 64 | Start: 2026-03-12 19:34:05.850000 | Status: running | Cmd: SELECT
...
-- Session: 64 | Start: 2026-03-12 19:49:21.720000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
  ...
  WHERE ((SP.CODPROD LIKE 'DES%') OR (SP.DESCRIPALL LIKE 'DES%') OR (SP.REFERE LIKE 'DES%') OR (SP.EXISTEN LIKE 'DES%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 35
GO
...
-- Session: 64 | Start: 2026-03-12 19:49:44.620000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
...
-- Session: 54 | Start: 2026-03-12 19:50:21.980000 | Status: running | Cmd: AWAITING COMMAND
SELECT N'Testing Connection...'
GO

-- Session: 64 | Start: 2026-03-12 19:56:04.657000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 64 | Start: 2026-03-12 19:56:07.957000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 64 | Start: 2026-03-12 19:58:12.633000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
...
-- Session: 64 | Start: 2026-03-12 19:58:46.283000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
...
-- Session: 67 | Start: 2026-03-12 20:00:02.413000 | Status: suspended | Cmd: UPDATE
...
-- Session: 64 | Start: 2026-03-12 20:10:42.470000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7594001450054' OR P.CodProd='7594001450054')
GO

-- Session: 64 | Start: 2026-03-12 20:10:50 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='788070552819' OR P.CodProd='788070552819')
GO

-- Session: 64 | Start: 2026-03-12 20:11:57.603000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7841141055447' OR P.CodProd='7841141055447')
GO
...
-- Session: 54 | Start: 2026-03-12 20:15:00.480000 | Status: runnable | Cmd: BACKUP DATABASE
CREATE PROCEDURE [dbo].[BackupEnterpriseAdmin_AMC]
AS
BEGIN
    SET NOCOUNT ON;
...
-- Session: 64 | Start: 2026-03-12 20:16:11.507000 | Status: runnable | Cmd: SELECT
...
WHERE ( SAFACT.CodSucu = '00000' )
       ...
       AND ( SAFACT.NumeroD = '44359' )
GO

-- Session: 54 | Start: 2026-03-12 20:17:40.460000 | Status: runnable | Cmd: SELECT
-- Query for 'Lotes' worksheet: filters lots based on entry date, rotation and quantity.
SELECT
    SALOTE.CodProd AS Cod,
    SALOTE.NroLote,
...
-- Session: 54 | Start: 2026-03-12 20:17:41.453000 | Status: suspended | Cmd: SELECT
SELECT 
    SAPROD.Descrip, 
    CASE 
        WHEN SATAXPRD.Monto IS NOT NULL THEN SAPROD.Precio1 * (1 + SATAXPRD.Monto / 100) 
...
-- Session: 62 | Start: 2026-03-12 20:22:08.357000 | Status: runnable | Cmd: SELECT
-- This script extracts inventory, costs, rotation, and expiration classification,
-- ensuring that the next expiration date (ProximaFechaV) is only taken from lots with active stock (Cantidad > 0).
...
-- Session: 62 | Start: 2026-03-12 20:22:15.873000 | Status: runnable | Cmd: SELECT
SELECT 
    SAPROD.Descrip, 
    CASE 
        WHEN SATAXPRD.Monto IS NOT NULL THEN SAPROD.Precio1 * (1 + SATAXPRD.Monto / 100) 
...
-- Session: 62 | Start: 2026-03-12 20:30:00.200000 | Status: suspended | Cmd: BACKUP DATABASE
CREATE PROCEDURE [dbo].[BackupEnterpriseAdmin_AMC]
AS
BEGIN
    SET NOCOUNT ON;
...
-- Session: 64 | Start: 2026-03-12 20:33:25.310000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 64 | Start: 2026-03-12 20:34:25.690000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 64 | Start: 2026-03-12 20:34:28.223000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 59 | Start: 2026-03-12 20:35:32.120000 | Status: running | Cmd: SELECT
-- Query for 'Lotes' worksheet: filters lots based on entry date, rotation and quantity.
SELECT
    SALOTE.CodProd AS Cod,
    SALOTE.NroLote,
...
-- Session: 64 | Start: 2026-03-12 20:36:10.353000 | Status: running | Cmd: EXECUTE
SELECT SAFACT.AutSRI, SAFACT.Cambio, 
       SAFACT.CancelA, SAFACT.CancelC, 
       SAFACT.CancelE, SAFACT.CancelG, 
       SAFACT.CancelI, SAFACT.CancelP, 
...
WHERE ( SAFACT.CodSucu = '00000' )
       AND ( SAFACT.TipoFac = 'A' )
       AND ( SAFACT.NumeroD = '44360' )
GO

-- Session: 64 | Start: 2026-03-12 20:57:54.543000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 64 | Start: 2026-03-12 20:58:00.610000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 64 | Start: 2026-03-12 21:09:05.720000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 64 | Start: 2026-03-12 21:17:54.027000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 59 | Start: 2026-03-12 21:18:19.700000 | Status: runnable | Cmd: SELECT
SELECT 
    SAPROD.Descrip, 
    CASE 
        WHEN SATAXPRD.Monto IS NOT NULL THEN SAPROD.Precio1 * (1 + SATAXPRD.Monto / 100) 
...
-- Session: 64 | Start: 2026-03-12 21:18:45.713000 | Status: runnable | Cmd: UPDATE
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
-- Session: 64 | Start: 2026-03-12 21:23:37.817000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 64 | Start: 2026-03-12 21:23:58.927000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CODPROD LIKE 'VIZ%') OR (SP.DESCRIPALL LIKE 'VIZ%') OR (SP.REFERE LIKE 'VIZ%') OR (SP.EXISTEN LIKE 'VIZ%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 35
GO

-- Session: 58 | Start: 2026-03-12 21:25:41.927000 | Status: running | Cmd: SELECT
SELECT 
    SAPROD.Descrip, 
    CASE 
        WHEN SATAXPRD.Monto IS NOT NULL THEN SAPROD.Precio1 * (1 + SATAXPRD.Monto / 100) 
...
-- Session: 64 | Start: 2026-03-12 21:51:34.720000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 64 | Start: 2026-03-12 21:54:22.117000 | Status: runnable | Cmd: SELECT
...
-- Session: 59 | Start: 2026-03-12 23:07:23.020000 | Status: running | Cmd: SELECT
SELECT * FROM Custom_Inventario_i360;
GO

-- Session: 54 | Start: 2026-03-12 23:30:19.550000 | Status: running | Cmd: EXECUTE
...
-- Session: 51 | Start: 2026-03-13 04:15:02.833000 | Status: running | Cmd: SELECT
...
-- Session: 54 | Start: 2026-03-13 06:31:00.830000 | Status: runnable | Cmd: INSERT
...
-- Session: 58 | Start: 2026-03-13 07:00:00.847000 | Status: suspended | Cmd: INSERT
CREATE PROCEDURE sp_sqlagent_log_jobhistory
  @job_id               UNIQUEIDENTIFIER,
  @step_id              INT,
  @sql_message_id       INT = 0,
...
-- Session: 60 | Start: 2026-03-13 07:00:00.760000 | Status: running | Cmd: SELECT
declare 
@Invdia as decimal = (SELECT total_inv FROM CUSTOM_INVENTARIO_DIVISAS), 
@pro as real = (SELECT COSTO_PROMEDIO FROM CUSTOM_INVENTARIO_DIVISAS),
@uni as decimal = (SELECT TOTAL_UNIDADES FROM CUSTOM_INVENTARIO_DIVISAS)
...
-- Session: 54 | Start: 2026-03-13 07:01:00.180000 | Status: running | Cmd: EXECUTE
xp_instance_regread
GO

-- Session: 61 | Start: 2026-03-13 07:03:15.447000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CODPROD LIKE 'HIERR%') OR (SP.DESCRIPALL LIKE 'HIERR%') OR (SP.REFERE LIKE 'HIERR%') OR (SP.EXISTEN LIKE 'HIERR%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO

-- Session: 61 | Start: 2026-03-13 07:03:59.420000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT  * , ROW_NUMBER() OVER (ORDER BY Descrip ASC) AS ROWNUM   FROM SACLIE WITH (NOLOCK) 
  WHERE (ACTIVO>0))
...
-- Session: 61 | Start: 2026-03-13 07:04:06.250000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CodProd LIKE 'COMPL%') OR (SP.DescripAll LIKE 'COMPL%') OR (SP.Refere LIKE 'COMPL%') OR (SP.Existen LIKE 'COMPL%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO

-- Session: 61 | Start: 2026-03-13 07:06:16.870000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='813333014541' OR P.CodProd='813333014541')
GO

-- Session: 61 | Start: 2026-03-13 07:07:11.683000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT DBO.FN_ADM_TAXPRODUCT('36596',339.352,1.00,0,0) MTOTAX
GO
...
-- Session: 61 | Start: 2026-03-13 07:13:51.773000 | Status: running | Cmd: SELECT
SELECT SAFACT.AutSRI, SAFACT.Cambio, 
       SAFACT.CancelA, SAFACT.CancelC, 
       SAFACT.CancelE, SAFACT.CancelG, 
       SAFACT.CancelI, SAFACT.CancelP, 
...
-- Session: 61 | Start: 2026-03-13 07:37:48.427000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
...
-- Session: 58 | Start: 2026-03-13 08:03:54.307000 | Status: running | Cmd: AWAITING COMMAND
(@P1 varchar(5),@P2 varchar(15))SET DATEFORMAT YMD;
SELECT DP.CODUBIC, DP.DESCRIP, SI.NROSERIAL
  FROM SASERI SI WITH (NOLOCK) 
       INNER JOIN SADEPO DP
       ON DP.CODUBIC=SI.CODUBIC
 WHERE SI.CODSUCU=@P1 AND SI.CODPROD=@P2
GO

-- Session: 58 | Start: 2026-03-13 08:03:56.730000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
...
-- Session: 58 | Start: 2026-03-13 08:04:05.720000 | Status: suspended | Cmd: SELECT
SELECT A.*
FROM SFTITM A
ORDER BY A.itemid ASC
GO

-- Session: 58 | Start: 2026-03-13 08:04:08.357000 | Status: running | Cmd: AWAITING COMMAND
SELECT SAFACT.NumeroD NumeroD_2, 
       SAFACT.TipoFac TipoFac_2, 
       SAITEMFAC.Cantidad, SAITEMFAC.CantidadU, 
       SAITEMFAC.CantMayor, SAITEMFAC.CodItem, 
...
-- Session: 66 | Start: 2026-03-13 08:13:40.423000 | Status: running | Cmd: SELECT
SELECT 
    SAPROD.Descrip, 
    CASE 
        WHEN SATAXPRD.Monto IS NOT NULL THEN SAPROD.Precio1 * (1 + SATAXPRD.Monto / 100) 
...
-- Session: 62 | Start: 2026-03-13 08:17:37.803000 | Status: running | Cmd: SELECT
-- Query for 'Lotes' worksheet: filters lots based on entry date, rotation and quantity.
SELECT
    SALOTE.CodProd AS Cod,
    SALOTE.NroLote,
...
-- Session: 54 | Start: 2026-03-13 08:21:31.567000 | Status: running | Cmd: AWAITING COMMAND
...
-- Session: 58 | Start: 2026-03-13 08:25:12.787000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 58 | Start: 2026-03-13 08:25:32.763000 | Status: running | Cmd: SELECT (STATMAN)
SET DATEFORMAT YMD;
...
-- Session: 51 | Start: 2026-03-13 08:25:38.437000 | Status: running | Cmd: SELECT
-- Query for 'Lotes' worksheet: filters lots based on entry date, rotation and quantity.
SELECT
    SALOTE.CodProd AS Cod,
    SALOTE.NroLote,
...
-- Session: 58 | Start: 2026-03-13 08:27:50.103000 | Status: running | Cmd: UPDATE
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
-- Session: 66 | Start: 2026-03-13 08:45:00.733000 | Status: runnable | Cmd: EXECUTE
xp_instance_regread
GO
...
-- Session: 61 | Start: 2026-03-13 08:50:11.077000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 62 | Start: 2026-03-13 08:50:27.080000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7593567000697' OR P.CodProd='7593567000697')
GO
...
-- Session: 60 | Start: 2026-03-13 09:00:31.300000 | Status: runnable | Cmd: SELECT
SELECT * FROM Custom_Inventario_i360;
GO
...
-- Session: 60 | Start: 2026-03-13 09:01:00.597000 | Status: runnable | Cmd: EXECUTE
xp_instance_regread
GO

-- Session: 67 | Start: 2026-03-13 09:03:00.443000 | Status: runnable | Cmd: UPDATE
UPDATE SAPROD 
...
-- Session: 67 | Start: 2026-03-13 09:03:01.230000 | Status: suspended | Cmd: UPDATE
...
-- Session: 61 | Start: 2026-03-13 09:05:04.863000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 62 | Start: 2026-03-13 09:06:34.920000 | Status: running | Cmd: SELECT
SELECT SAFACT.AutSRI, SAFACT.Cambio, 
       SAFACT.CancelA, SAFACT.CancelC, 
       SAFACT.CancelE, SAFACT.CancelG, 
       SAFACT.CancelI, SAFACT.CancelP, 
...
-- Session: 61 | Start: 2026-03-13 09:12:26.500000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 65 | Start: 2026-03-13 09:17:31.540000 | Status: runnable | Cmd: SELECT
-- This script extracts inventory, costs, rotation, and expiration classification,
-- ensuring that the next expiration date (ProximaFechaV) is only taken from lots with active stock (Cantidad > 0).
...
-- Session: 61 | Start: 2026-03-13 09:21:31.903000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 61 | Start: 2026-03-13 09:22:15.740000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 69 | Start: 2026-03-13 09:33:38.347000 | Status: running | Cmd: SELECT
-- Query for 'Lotes' worksheet: filters lots based on entry date, rotation and quantity.
SELECT
    SALOTE.CodProd AS Cod,
    SALOTE.NroLote,
...
-- Session: 69 | Start: 2026-03-13 10:03:00.103000 | Status: running | Cmd: UPDATE
UPDATE SAPROD 
SET PrecioI1=b.precio$1,PrecioI2=b.precio$2,PrecioI3=b.precio$3
from SAPROD as a
inner join CUSTOM_PRECIO_EN_DOLAR as b on (a.CodProd=b.codprod)
...
-- Session: 61 | Start: 2026-03-13 10:03:43.730000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT  * , ROW_NUMBER() OVER (ORDER BY CodServ ASC) AS ROWNUM   FROM VW_ADM_SERVICIOS WITH (NOLOCK) 
  WHERE ((CodServ LIKE 'NINAZ%') OR (DescripAll LIKE 'NINAZ%') OR (Clase LIKE 'NINAZ%')) AND (ACTIVO=1) AND (EsVenta=1))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
...
-- Session: 51 | Start: 2026-03-13 10:10:58.150000 | Status: suspended | Cmd: SELECT
...
-- Session: 65 | Start: 2026-03-13 10:30:00.457000 | Status: suspended | Cmd: UPDATE
...
-- Session: 69 | Start: 2026-03-13 10:30:00.450000 | Status: suspended | Cmd: UPDATE
...
-- Session: 59 | Start: 2026-03-13 10:39:37.920000 | Status: runnable | Cmd: SELECT
...
-- Session: 64 | Start: 2026-03-13 10:45:52.630000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 60 | Start: 2026-03-13 10:49:38.640000 | Status: runnable | Cmd: SELECT
SELECT 
    SAPROD.Descrip, 
    CASE 
        WHEN SATAXPRD.Monto IS NOT NULL THEN SAPROD.Precio1 * (1 + SATAXPRD.Monto / 100) 
...
-- Session: 64 | Start: 2026-03-13 10:49:40.103000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
 INNER JOIN SAEXIS EX ON (EX.CodSucu='00000') And (EX.CodProd=SP.CodProd) And (EX.CodUbic='AMR001')
  WHERE ((SP.CODPROD LIKE 'VA%') OR (SP.DESCRIPALL LIKE 'VA%') OR (SP.REFERE LIKE 'VA%') OR (SP.EXISTEN LIKE 'VA%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO

-- Session: 64 | Start: 2026-03-13 10:51:34.240000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
 INNER JOIN SAEXIS EX ON (EX.CodSucu='00000') And (EX.CodProd=SP.CodProd) And (EX.CodUbic='AMR001')
  WHERE ((SP.CODPROD LIKE '7591585119100%') OR (SP.DESCRIPALL LIKE '7591585119100%') OR (SP.REFERE LIKE '7591585119100%') OR (SP.EXISTEN LIKE '7591585119100%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO
...
-- Session: 61 | Start: 2026-03-13 10:59:39.533000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
...
-- Session: 62 | Start: 2026-03-13 10:59:59.183000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 61 | Start: 2026-03-13 11:05:15.567000 | Status: running | Cmd: SET COMMAND
SET DATEFORMAT YMD;
SELECT * FROM SALOTE WITH (NOLOCK)  WHERE (CodSucu='00000') And (CodProd='7592454146807') And (CodUbic='AMR001') AND ((Cantidad>0) or (CantidadU>0))ORDER BY FechaE
GO
...
-- Session: 61 | Start: 2026-03-13 11:18:54.327000 | Status: running | Cmd: SET COMMAND
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 61 | Start: 2026-03-13 11:21:51.590000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 61 | Start: 2026-03-13 11:22:05.190000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
...
-- Session: 62 | Start: 2026-03-13 11:26:45.573000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 62 | Start: 2026-03-13 11:28:56.810000 | Status: running | Cmd: AWAITING COMMAND
SET DATEFORMAT YMD;
SELECT * FROM VW_ADM_TAXINVENT WITH (NOLOCK) WHERE  (ESTAXVENTA>0) AND CodProd='196852644438'
GO

-- Session: 66 | Start: 2026-03-13 11:30:31.233000 | Status: running | Cmd: SELECT
SELECT * FROM Custom_Inventario_i360;
GO

-- Session: 61 | Start: 2026-03-13 11:32:11.627000 | Status: runnable | Cmd: SELECT (STATMAN)
SET DATEFORMAT YMD;
SELECT lo.nrounico, lo.nrolote, lo.codubic, dp.descrip, lo.costo, Precio3 AS PRECIO, dbo.FN_ADM_TAXPRODUCT(CodProd,Precio3,1,0,0)+Precio3 As PrecioTx,PrecioU3 AS PRECIOU, dbo.FN_ADM_TAXPRODUCT(CodProd,PrecioU3,1,1,0)+PrecioU3 As PrecioUTx,lo.cantidad, lo.cantidadu, lo.fechae, lo.fechav from SALOTE lo Inner Join sadepo dp on lo.codubic=dp.codubic where (lo.codprod='8908020242542') And (lo.CodUbic='AMR001')  And ((lo.Cantidad>0) Or (lo.CantidadU>0)) Order By lo.codubic, lo.fechav
GO

-- Session: 62 | Start: 2026-03-13 11:33:32.653000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT  * , ROW_NUMBER() OVER (ORDER BY Descrip ASC) AS ROWNUM   FROM SACLIE WITH (NOLOCK) 
...
-- Session: 61 | Start: 2026-03-13 11:44:57.517000 | Status: runnable | Cmd: SELECT
(@P1 varchar(15))SELECT *
  FROM SAIPRD IG WITH (NOLOCK)
 WHERE IG.CODPROD=@P1
GO
...
-- Session: 61 | Start: 2026-03-13 11:49:17.913000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 61 | Start: 2026-03-13 11:49:51.603000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 61 | Start: 2026-03-13 11:55:25.570000 | Status: running | Cmd: EXECUTE
SELECT SAFACT.AutSRI, SAFACT.Cambio, 
       SAFACT.CancelA, SAFACT.CancelC, 
       SAFACT.CancelE, SAFACT.CancelG, 
       SAFACT.CancelI, SAFACT.CancelP, 
...
WHERE ( SAFACT.CodSucu = '00000' )
       AND ( SAFACT.TipoFac = 'A' )
       AND ( SAFACT.NumeroD = '44382' )
GO

-- Session: 61 | Start: 2026-03-13 11:57:43.540000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 69 | Start: 2026-03-13 12:25:14.550000 | Status: running | Cmd: SELECT
(@P1 nvarchar(22),@P2 nvarchar(16))
            SELECT 
            ...
            WHERE cxp.CodProv = @P1 AND cxp.NumeroD = @P2 AND cxp.TipoCxP = '10'
GO
...
-- Session: 69 | Start: 2026-03-13 12:25:35.617000 | Status: running | Cmd: SELECT
(@P1 nvarchar(4),@P2 nvarchar(4),@P3 nvarchar(4),@P4 nvarchar(4))
            SELECT
              SACOMP.FechaI,
              SACOMP.FechaE,
...
-- Session: 65 | Start: 2026-03-13 12:29:39.380000 | Status: suspended | Cmd: SELECT
SELECT 
    SAPROD.Descrip, 
    CASE 
        WHEN SATAXPRD.Monto IS NOT NULL THEN SAPROD.Precio1 * (1 + SATAXPRD.Monto / 100) 
...
-- Session: 64 | Start: 2026-03-13 12:29:50.930000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
 INNER JOIN SAEXIS EX ON (EX.CodSucu='00000') And (EX.CodProd=SP.CodProd) And (EX.CodUbic='AMR001')
  WHERE ((SP.CODPROD LIKE '7598252000211%') OR (SP.DESCRIPALL LIKE '7598252000211%') OR (SP.REFERE LIKE '7598252000211%') OR (SP.EXISTEN LIKE '7598252000211%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO

-- Session: 61 | Start: 2026-03-13 12:33:28.033000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='AMP_VIT_C_500' OR P.CodProd='AMP_VIT_C_500')
GO

-- Session: 61 | Start: 2026-03-13 12:34:40.147000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 61 | Start: 2026-03-13 12:35:04.240000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 60 | Start: 2026-03-13 12:45:01.163000 | Status: suspended | Cmd: COMMIT TRANSACTION
...
-- Session: 62 | Start: 2026-03-13 12:51:14.870000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='FESTAL' OR P.CodProd='FESTAL')
GO
...
-- Session: 62 | Start: 2026-03-13 12:51:49.147000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 72 | Start: 2026-03-13 12:53:32.033000 | Status: runnable | Cmd: SELECT
-- This script extracts inventory, costs, rotation, and expiration classification,
-- ensuring that the next expiration date (ProximaFechaV) is only taken from lots with active stock (Cantidad > 0).
...
-- Session: 65 | Start: 2026-03-13 13:01:00.157000 | Status: suspended | Cmd: UPDATE
UPDATE SAPROD
SET Refere=b.precio$
from SAPROD as a
inner join CUSTOM_COSTO_COMPRAS as b on (a.CodProd=b.codprod)
...
-- Session: 65 | Start: 2026-03-13 13:01:37.237000 | Status: runnable | Cmd: SELECT
-- Query for 'Lotes' worksheet: filters lots based on entry date, rotation and quantity.
SELECT
    SALOTE.CodProd AS Cod,
    SALOTE.NroLote,
...
-- Session: 64 | Start: 2026-03-13 13:01:42.640000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT  * , ROW_NUMBER() OVER (ORDER BY Descrip DESC) AS ROWNUM   FROM SACLIE WITH (NOLOCK) 
  WHERE ((CodClie LIKE 'NA%') OR (Descrip LIKE 'NA%') OR (ID3 LIKE 'NA%') OR (Clase LIKE 'NA%') OR (Saldo LIKE 'NA%')) AND (ACTIVO>0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
...
-- Session: 64 | Start: 2026-03-13 13:01:53.340000 | Status: runnable | Cmd: SET COMMAND
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
 INNER JOIN SAEXIS EX ON (EX.CodSucu='00000') And (EX.CodProd=SP.CodProd) And (EX.CodUbic='AMR001')
  WHERE ((SP.CODPROD LIKE '7591821101692%') OR (SP.DESCRIPALL LIKE '7591821101692%') OR (SP.REFERE LIKE '7591821101692%') OR (SP.EXISTEN LIKE '7591821101692%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO
...
-- Session: 64 | Start: 2026-03-13 13:03:24.237000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SAEXIS EX     ON (EX.CODSUCU='00000') AND        (EX.CODPROD=P.CODPROD) AND        (EX.CODUBIC='AMR001') INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='ALKA' OR P.CodProd='ALKA')
GO

-- Session: 64 | Start: 2026-03-13 13:04:33.670000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
 INNER JOIN SAEXIS EX ON (EX.CodSucu='00000') And (EX.CodProd=SP.CodProd) And (EX.CodUbic='AMR001')
  WHERE ((SP.CODPROD LIKE '7800004500260%') OR (SP.DESCRIPALL LIKE '7800004500260%') OR (SP.REFERE LIKE '7800004500260%') OR (SP.EXISTEN LIKE '7800004500260%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO
...
-- Session: 61 | Start: 2026-03-13 13:13:32.400000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
...
-- Session: 62 | Start: 2026-03-13 13:20:35.303000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT P.*, C.Saldo AS SALDOP,
       DBO.FN_ADM_DESCTOCONVENIO('V16923739',7281.41,4.00,'2026-03-13 12:47:20.504') AS DESCTOCV 
  FROM SACLIE P 
  LEFT JOIN (SELECT CODCLIE, SUM(SALDO) AS SALDO 
               FROM SAACXC
              WHERE (SALDO>0) AND ((TIPOCXC IN ('10','60','70')) or (substring(tipocxC,1,1)='2'))
              GROUP BY CODCLIE) C ON
       P.CODCLIE=C.CODCLIE
 WHERE P.CODCLIE='V16923739'
GO

-- Session: 62 | Start: 2026-03-13 13:20:48.437000 | Status: running | Cmd: SELECT
SELECT SAFACT.AutSRI, SAFACT.Cambio, 
       SAFACT.CancelA, SAFACT.CancelC, 
       SAFACT.CancelE, SAFACT.CancelG, 
       SAFACT.CancelI, SAFACT.CancelP, 
...
WHERE ( SAFACT.CodSucu = '00000' )
       AND ( SAFACT.TipoFac = 'A' )
       AND ( SAFACT.NumeroD = '44386' )
GO

-- Session: 61 | Start: 2026-03-13 13:23:26.590000 | Status: running | Cmd: SELECT
SELECT SAFACT.AutSRI, SAFACT.Cambio, 
       SAFACT.CancelA, SAFACT.CancelC, 
       SAFACT.CancelE, SAFACT.CancelG, 
       SAFACT.CancelI, SAFACT.CancelP, 
...
WHERE ( SAFACT.CodSucu = '00000' )
       AND ( SAFACT.TipoFac = 'A' )
       AND ( SAFACT.NumeroD = '44387' )
GO

-- Session: 62 | Start: 2026-03-13 13:26:24.803000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 62 | Start: 2026-03-13 13:27:34.333000 | Status: running | Cmd: EXECUTE
SET DATEFORMAT YMD;
SELECT P.*, C.Saldo AS SALDOP,
       DBO.FN_ADM_DESCTOCONVENIO('V10915197',1400.67,1.00,'2026-03-13 13:11:21.660') AS DESCTOCV 
  FROM SACLIE P 
  LEFT JOIN (SELECT CODCLIE, SUM(SALDO) AS SALDO 
               FROM SAACXC
              WHERE (SALDO>0) AND ((TIPOCXC IN ('10','60','70')) or (substring(tipocxC,1,1)='2'))
              GROUP BY CODCLIE) C ON
       P.CODCLIE=C.CODCLIE
 WHERE P.CODCLIE='V10915197'
GO

-- Session: 65 | Start: 2026-03-13 13:29:37.727000 | Status: running | Cmd: SELECT
...
-- Session: 59 | Start: 2026-03-13 13:29:43.723000 | Status: running | Cmd: UPDATE
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
BEGIN TRANSACTION;
BEGIN TRY
SET @NUMEROCOM='B0311845'
UPDATE SAEXIS SET 
       [UnidPed]=[UnidPed]+-2.00
 WHERE (CodSucu='00000') And (CodProd='7592430000185') And (CodUbic='AMR001')
SET @NUMERRORS=@NUMERRORS+@@ERROR;
UPDATE SAPROD SET 
       @UCOSTOACT=COSTACT, @UCOSTOANT=COSTANT, @UCOSTOPRO=COSTPRO, CostAnt=CostAct,
CostPro=ISNULL((CASE WHEN COSTPRO=0 THEN 2142.05 
ELSE ((CostPro*Existen)+4284.10)/NULLIF(Existen+2.00,0) END),0), 
COSTACT=2142.05,@NCOSTOACT=CostAct,@NCOSTOANT=CostAnt,@NCOSTOPRO=CostPro,FechaUC='2026-03-12 13:29:43.651'
 WHERE (CodProd='7592430000185')
UPDATE SAPROD
  ...
  FROM SAPART 
 WHERE (SAPART.CODPROD=SAPROD.CODPROD) AND (SAPART.CODALTE='7592430000185')
SELECT @EXISTANT=EXISTEN, @EXISTANTUND=EXUNIDAD   FROM SAEXIS WITH (NOLOCK)   WHERE CODPROD='7592430000185' AND   CODSUCU='00000' AND   CODUBIC='AMR001';
EXEC TR_ADM_UPDATE_EXISTENCIAS '00000','7592430000185','AMR001',2.00,0,'2026-03-12'
INSERT INTO SALOTE ([CodSucu],[CodProd],[CodUbic],[NroLote])
       VALUES ('00000','7592430000185','AMR001','987')
SET @NROUNICOLOT=IDENT_CURRENT('SALOTE')
UPDATE SALOTE SET 
       [Cantidad]=[Cantidad]+2.00, PrecioU1=0.00,PrecioU2=0.00,PrecioU3=0.00,Precio1=2142.05,Precio2=2142.05,Precio3=2142.05,Costo=2142.05,FechaE='2026-03-12',FechaV='1899-12-30'
 WHERE NroUnico=@NROUNICOLOT
UPDATE SALOTE SET 
       PrecioU1=0.00,PrecioU2=0.00,PrecioU3=0.00
 WHERE (CodSucu='00000') And (CodProd='7592430000185') And (CodUbic='AMR001')
UPDATE SALOTE SET 
       Precio1=2142.05,Precio2=2142.05,Precio3=2142.05
 WHERE (CodSucu='00000') And (CodProd='7592430000185') And (CodUbic='AMR001')
IF NOT EXISTS(SELECT * FROM SAPVPR WITH (NOLOCK)
               WHERE (TipoCom='H') And 
                     (CodItem='7592430000185') And 
                     (CodProv='J-412413740'))
   INSERT INTO SAPVPR (TipoCom,NumeroD,CodItem,CodProv)
          VALUES ('H',@NUMEROCOM,'7592430000185','J-412413740');
...
INSERT INTO SAITEMCOM ([Signo],[CodSucu],[CodProv],[TipoCom],[NumeroD],[NroLinea],[FechaE],[CodItem],[Refere],[CodUbic],[Descrip1],[Cantidad],[Costo],[Precio1],[Precio2],[Precio3],[TotalItem],[EsExento],[DEsLote],[NroUnicoL],[NroLote],[FechaL],[ExistAntU],[ExistAnt])
       VALUES (1,'00000','J-412413740','H',@NUMEROCOM,1,'2026-03-12 13:29:43.651','7592430000185','6.06542','AMR001','CENTELLA ASIATICA X60 CAP NATURAL PREMIU',2.00,2142.05,2142.05,2142.05,2142.05,4284.10,1,1,ISNULL(@NROUNICOLOT,0),'987','2026-03-12 13:29:43.651',@EXISTANTUND,@EXISTANT)
INSERT INTO SACOMP ([Signo],[TipoCom],[CodSucu],[CodUsua],[CodEsta],[FechaT],[FechaI],[FechaE],[FechaV],[NumeroD],[CodProv],[CodUbic],[Descrip],[Factor],[MontoMEx],[NroCtrol],[ID3],[MtoTotal],[Monto],[TExento],[TotalPrd],[OrdenC],[CodOper],[Credito])
       VALUES (1,'H','00000','V12400678','ADM-3',GETDATE(),'2026-03-13 13:29:43.651','2026-03-12 13:29:43.651','2026-04-02 13:29:43.651',@NUMEROCOM,'J-412413740','AMR001','INSUAMINCA C.A.',443.25,9.6652,'00-1138023','J-412413740',4284.10,4284.10,4284.10,4284.10,'B0311845','CXP',4284.10)
UPDATE SAPROV SET 
       FechaUC='2026-03-13', MontoUC=4284.10, NumeroUC='B0311845', [RetenIVA]=[RetenIVA]+0.00
...
INSERT INTO SAACXP ([CodSucu],[CodProv],[NumeroD],[NroCtrol],[CodUsua],[CodEsta],[TipoCxP],[Descrip],[ID3],[FechaT],[Document],[FechaI],[FechaE],[FechaV],[Factor],[MontoMEx],[SaldoMEx],[Monto],[MontoNeto],[Saldo],[SaldoOrg],[TExento],[EsLibroI],[CodOper])
       VALUES ('00000','J-412413740','B0311845','00-1138023','V12400678','ADM-3','10','INSUAMINCA C.A.','J-412413740',GETDATE(),'B0311845 B0311845','2026-03-13 13:29:43.651','2026-03-12 13:29:43.651','2026-04-02 13:29:43.651',443.25,9.6652,9.6652,4284.10,4284.10,4284.10,4284.10,4284.10,1,'CXP')
...
-- Session: 60 | Start: 2026-03-13 13:31:01.223000 | Status: suspended | Cmd: UPDATE
CREATE PROCEDURE [dbo].[sp_sqlagent_update_jobactivity_next_scheduled_date]
    @session_id            INT,
    @job_id                UNIQUEIDENTIFIER,
	@is_system             TINYINT = 0,
...
-- Session: 59 | Start: 2026-03-13 13:32:44.267000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT DBO.FN_ADM_PRICEBASE('8906082150973',680.79,0,1) AS PRECIOB
GO

-- Session: 59 | Start: 2026-03-13 13:33:54.683000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 *, ISNULL((SELECT COUNT(CODALTE) FROM SAEQUI WITH (NOLOCK)  WHERE CODPROD='7590027002673'),0) AS CANTEQUIV,  0 AS ISADIC FROM VW_ADM_PRODUCTOS with (nolock) WHERE (CodProd='7590027002673') And (Activo=1)
GO
...
-- Session: 62 | Start: 2026-03-13 13:39:13.823000 | Status: running | Cmd: SELECT
SELECT TOP 1 CodProd   FROM VW_ADM_PRODUCTOS WITH (NOLOCK)
GO

-- Session: 65 | Start: 2026-03-13 13:40:54.337000 | Status: running | Cmd: SET COMMAND
SET DATEFORMAT YMD;
Select P.parametro, p.activo, p.clave, p.habilitado, 
       isNull(d.nodata,0) as NoData, d.tipodata, d.idata, d.sdata, d.fdata 
  from SSPARM P WITH (NOLOCK) 
  left join sspard D On
        p.Parametro=d.Parametro and p.Modulo=d.Modulo and p.CodParm=d.CodParm 
  Where p.CodParm='_U_DARIO' And p.Modulo=901
Order By p.Parametro
GO

-- Session: 65 | Start: 2026-03-13 13:40:54.813000 | Status: suspended | Cmd: SELECT
select (select COUNT(codprov) from SAPROV where Activo>0) as cntProv,
(select COUNT(CodUbic) from SADEPO where Activo>0) as cntDepo,
(select COUNT(CodInst) from SAINSTA where TipoIns=0) as cntInsP,
(select COUNT(CodProd) from SAPROD where Activo>0) as cntProd,
(select COUNT(CodZona) from SAZONA where Activo>0) as cntZona,
(select COUNT(CodVend) from SAVEND where Activo>0) as cntVend,
(select COUNT(CodInst) from SAINSTA where TipoIns=1) as cntInsS,
(select COUNT(CodServ) from SASERV where Activo>0) as cntServ,
(select COUNT(Codfacl) from SAFALO               ) as cntFACL,
(select COUNT(CodConv) from SACONV where Activo>0) as cntConv,
(select COUNT(CodClie) from SACLIE where Activo>0) as cntClie,
(select COUNT(CodMeca) from SAMECA where Activo>0) as cntMeca,
(select COUNT(CodTarj) from SATARJ where Activo>0) as cntTarj,
(select COUNT(CodOper) from SAOPER where Activo>0) as cntOper,
(select COUNT(NumeroD) from SAOFER where Activo>0) as cntOfer,
(select COUNT(CodTaxs) from SATAXES where Activo>0 AND ESRETEN>0) as cntReten;
GO

-- Session: 65 | Start: 2026-03-13 13:41:25.500000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT EX.Cantidad As Existen, EX.CantidadU As ExUnidad, 0 as cantcom, 0 as unidcom,        ISNULL((SELECT COUNT(CODUBIC) FROM SAEXIS with (nolock)                 WHERE (CODSUCU='00000') AND                       (CODPROD='8904278589569') AND                       (CODUBIC='AMR001')),0) AS EXISTDEPO FROM SALOTE EX with (nolock) WHERE EX.CODUBIC='AMR001' AND EX.CODSUCU='00000' AND EX.CODPROD='8904278589569' AND EX.NroUnico=1047388
GO

-- Session: 65 | Start: 2026-03-13 13:41:47.087000 | Status: running | Cmd: UPDATE
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
 ...
 ,@EXISTANT     DECIMAL(28,3)=0
 ,@EXISTANTUND  DECIMAL(28,3)=0
 ,@EXISTANT2    DECIMAL(28,3)=0
 ,@EXISTANTUND2 DECIMAL(28,3)=0
  ,@NUMERRORS INT=0;
BEGIN TRANSACTION;
BEGIN TRY
EXEC SP_ADM_PROXCORREL '00000','','PrxDesc',@NUMEROOPI OUTPUT;
IF NOT EXISTS(SELECT NUMEROD FROM SAOPEI WITH (NOLOCK) 
               WHERE CODSUCU='00000' AND 
                     TIPOOPI='P' AND NUMEROD=@NUMEROOPI)
   BEGIN
UPDATE SAOPEI SET 
       [TipoOpI]='P',[NumeroD]=@NUMEROOPI,[Signo]=-1,[CodSucu]='00000',[CodEsta]='DARIOWIN11',[CodUsua]='DARIO',[Autori]='dario',[Respon]='dario',[UsoMat]='',[CodUbic]='AMR001',[Monto]=339.27,[FechaE]='2026-03-13 13:41:46.374',[FechaT]=GETDATE(),[CodOper]='DESC06',[UsoInterno]=0
 WHERE (CodSucu='00000') And (TipoOpI='P') And (NumeroD=@NUMEROOPI)
IF @@ROWCOUNT=0
INSERT INTO SAOPEI ([TipoOpI],[NumeroD],[Signo],[CodSucu],[CodEsta],[CodUsua],[Autori],[Respon],[CodUbic],[Monto],[FechaE],[FechaT],[CodOper])
       VALUES ('P',@NUMEROOPI,-1,'00000','DARIOWIN11','DARIO','dario','dario','AMR001',339.27,'2026-03-13 13:41:46.374',GETDATE(),'DESC06')
SELECT @EXISTANT=EXISTEN, @EXISTANTUND=EXUNIDAD   FROM SAEXIS WITH (NOLOCK)   WHERE CODPROD='8904278589569' AND   CODSUCU='00000' AND   CODUBIC='AMR001';
EXEC TR_ADM_UPDATE_EXISTENCIAS '00000','8904278589569','AMR001',-1.00,0,'2026-03-13'
SET @NROUNICOLOT=1047388
UPDATE SALOTE SET 
       [Cantidad]=[Cantidad]+-1.00
 WHERE NroUnico=@NROUNICOLOT
INSERT INTO SAITEMOPI ([CodSucu],[TipoOpI],[NumeroD],[NroLinea],[FechaE],[CodItem],[Refere],[CodUbic],[Signo],[Descrip1],[Cantidad],[Costo],[DEsLote],[NroLote],[NroUnicoL],[TotalItem],[FechaL],[ExistAnt],[ExistAntU])
       VALUES ('00000','P',@NUMEROOPI,1,'2026-03-13 13:41:46.374','8904278589569','1.45814','AMR001',1,'OLMESARTAN 40       MG X 30 TAB WEST-C',1.00,339.27,1,'4',ISNULL(@NROUNICOLOT,0),339.27,'2026-03-13 13:40:43.834',ISNULL(@EXISTANT,0),ISNULL(@EXISTANTUND,0))
   END
ELSE
  SET  @NUMERRORS=1004;
  IF @NUMERRORS>0
  ...
  END;
  COMMIT TRANSACTION;
  SELECT @NUMERRORS error, ISNULL(@NUMEROOPI,'') AS numerod;
END TRY
BEGIN CATCH
  IF (@@TRANCOUNT>0)
     ROLLBACK;
...
-- Session: 59 | Start: 2026-03-13 13:44:09.343000 | Status: running | Cmd: UPDATE
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
-- Session: 71 | Start: 2026-03-13 13:45:42.940000 | Status: suspended | Cmd: SELECT
SELECT 
    SAPROD.Descrip, 
    CASE 
        WHEN SATAXPRD.Monto IS NOT NULL THEN SAPROD.Precio1 * (1 + SATAXPRD.Monto / 100) 
...
-- Session: 64 | Start: 2026-03-13 13:46:10.067000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SAEXIS EX     ON (EX.CODSUCU='00000') AND        (EX.CODPROD=P.CODPROD) AND        (EX.CODUBIC='AMR001') INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7594001101949' OR P.CodProd='7594001101949')
GO

-- Session: 59 | Start: 2026-03-13 13:48:26.057000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CodProd LIKE 'ACEITE%') OR (SP.DescripAll LIKE 'ACEITE%') OR (SP.Refere LIKE 'ACEITE%') OR (SP.Existen LIKE 'ACEITE%')) AND  (SP.Activo=1))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO

-- Session: 64 | Start: 2026-03-13 13:48:50.670000 | Status: running | Cmd: SELECT
(@P1 int)SELECT A.*
FROM SAMUNICIPIO A
WHERE (A.Ciudad = @P1)
ORDER BY A.Ciudad ASC, A.Descrip ASC, A.Municipio ASC
GO

-- Session: 61 | Start: 2026-03-13 13:49:18.300000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT lo.nrounico, lo.nrolote, lo.codubic, dp.descrip, lo.costo, Precio3 AS PRECIO, dbo.FN_ADM_TAXPRODUCT(CodProd,Precio3,1,0,0)+Precio3 As PrecioTx,PrecioU3 AS PRECIOU, dbo.FN_ADM_TAXPRODUCT(CodProd,PrecioU3,1,1,0)+PrecioU3 As PrecioUTx,lo.cantidad, lo.cantidadu, lo.fechae, lo.fechav from SALOTE lo Inner Join sadepo dp on lo.codubic=dp.codubic where (lo.codprod='7702010972287') And (lo.CodUbic='AMR001')  And ((lo.Cantidad>0) Or (lo.CantidadU>0)) Order By lo.codubic, lo.fechav
GO
...
-- Session: 64 | Start: 2026-03-13 13:50:44.043000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT * FROM VW_ADM_TAXINVENT WITH (NOLOCK) WHERE  (ESTAXVENTA>0) AND CodProd='7592806112047'
GO

-- Session: 59 | Start: 2026-03-13 13:52:52.870000 | Status: runnable | Cmd: UPDATE
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
-- Session: 59 | Start: 2026-03-13 13:56:58.850000 | Status: suspended | Cmd: UPDATE
(@1 varchar(8000),@2 varchar(8000),@3 varchar(8000),@4 varchar(8000),@5 varchar(8000),@6 varchar(8000))UPDATE [SAEXIS] set [CodSucu] = @1,[CodProd] = @2,[CodUbic] = @3  WHERE [CodSucu]=@4 AND [CodProd]=@5 AND [CodUbic]=@6
GO

-- Session: 59 | Start: 2026-03-13 13:58:30.837000 | Status: suspended | Cmd: INSERT
(@1 varchar(8000),@2 varchar(8000),@3 varchar(8000))INSERT INTO [SAEXIS]([CodSucu],[CodProd],[CodUbic]) values(@1,@2,@3)
GO
...
-- Session: 64 | Start: 2026-03-13 14:01:12.160000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SAEXIS EX     ON (EX.CODSUCU='00000') AND        (EX.CODPROD=P.CODPROD) AND        (EX.CODUBIC='AMR001') INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='6948122306027' OR P.CodProd='6948122306027')
GO

-- Session: 59 | Start: 2026-03-13 14:01:12.667000 | Status: running | Cmd: SELECT
(@P1 varchar(15))SELECT *
  FROM SAIPRD IG WITH (NOLOCK)
 WHERE IG.CODPROD=@P1
GO

-- Session: 59 | Start: 2026-03-13 14:01:50.837000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 59 | Start: 2026-03-13 14:02:15.937000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
 ...
 WHERE (P.Activo=1) AND
       ((P.CodProd='JER') OR         (C.CodAlte='JER') OR 
        (P.Refere ='JER'))
GO
...
-- Session: 59 | Start: 2026-03-13 14:02:49.680000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 *, ISNULL((SELECT COUNT(CODALTE) FROM SAEQUI WITH (NOLOCK)  WHERE CODPROD='JER_5CC'),0) AS CANTEQUIV,  0 AS ISADIC FROM VW_ADM_PRODUCTOS with (nolock) WHERE (CodProd='JER_5CC') And (Activo=1)
GO

-- Session: 71 | Start: 2026-03-13 14:03:01.250000 | Status: running | Cmd: EXECUTE
SELECT ISNULL(SUSER_SNAME(), SUSER_NAME())
GO

-- Session: 61 | Start: 2026-03-13 14:03:09.753000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 67 | Start: 2026-03-13 14:10:19.477000 | Status: suspended | Cmd: INSERT
CREATE PROCEDURE sp_sqlagent_log_jobhistory
  @job_id               UNIQUEIDENTIFIER,
  @step_id              INT,
  @sql_message_id       INT = 0,
...
-- Session: 65 | Start: 2026-03-13 14:13:59.750000 | Status: running | Cmd: UPDATE
SET DATEFORMAT YMD;
DELETE SATAXPRD WHERE (CodProd='100117') AND not exists(select codtaxs from sataxes where codtaxs=SATAXPRD.codtaxs and esreten=1)
UPDATE SACODBAR SET 
       [CodProd]='100117',[CodAlte]='100117'
 WHERE (CodProd='100117') And (CodAlte='100117')
IF @@ROWCOUNT=0
INSERT INTO SACODBAR ([CodProd],[CodAlte])
       VALUES ('100117','100117')
UPDATE SAPROD SET SAPROD.COSTACT= SAPROD.COSTACT-(428.83)+428.83, SAPROD.COSTANT= SAPROD.COSTANT-(331.75)+331.75, SAPROD.COSTPRO= SAPROD.COSTPRO-(349.3014)+349.3014 FROM SAPART WHERE SAPART.CODPROD=SAPROD.CODPROD AND SAPART.CODALTE='100117'
GO
...
-- Session: 61 | Start: 2026-03-13 14:15:15.747000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT lo.nrounico, lo.nrolote, lo.codubic, dp.descrip, lo.costo, Precio3 AS PRECIO, dbo.FN_ADM_TAXPRODUCT(CodProd,Precio3,1,0,0)+Precio3 As PrecioTx,PrecioU3 AS PRECIOU, dbo.FN_ADM_TAXPRODUCT(CodProd,PrecioU3,1,1,0)+PrecioU3 As PrecioUTx,lo.cantidad, lo.cantidadu, lo.fechae, lo.fechav from SALOTE lo Inner Join sadepo dp on lo.codubic=dp.codubic where (lo.codprod='7591062010753') And (lo.CodUbic='AMR001')  And ((lo.Cantidad>0) Or (lo.CantidadU>0)) Order By lo.codubic, lo.fechav
GO

-- Session: 65 | Start: 2026-03-13 14:15:56.470000 | Status: running | Cmd: SET COMMAND
SET DATEFORMAT YMD;
Set DateFormat YMD
...
-- Session: 59 | Start: 2026-03-13 14:17:21.493000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT DBO.FN_ADM_PRICEBASE('JER_5CC',92.19,0,1) AS PRECIOB
GO

-- Session: 64 | Start: 2026-03-13 14:22:18.380000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
 INNER JOIN SAEXIS EX ON (EX.CodSucu='00000') And (EX.CodProd=SP.CodProd) And (EX.CodUbic='AMR001')
  WHERE ((SP.CodProd LIKE '%RECOLE%') OR (SP.DescripAll LIKE '%RECOLE%') OR (SP.Refere LIKE '%RECOLE%') OR (SP.Existen LIKE '%RECOLE%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO

-- Session: 71 | Start: 2026-03-13 14:24:22.190000 | Status: suspended | Cmd: SELECT
SELECT asf.assembly_id, asi.name, asf.content FROM [sys].[assembly_files] asf INNER JOIN [sys].[assemblies] asi ON asi.assembly_id = asf.assembly_id
GO

-- Session: 65 | Start: 2026-03-13 14:28:14.470000 | Status: running | Cmd: SELECT
SELECT EsPorct,Monto FROM VW_ADM_TAXINVENT WITH (NOLOCK) WHERE (CodProd='18904187883892') And (EsReten=0)
GO
...
-- Session: 59 | Start: 2026-03-13 14:32:02.407000 | Status: runnable | Cmd: SELECT
(@1 varchar(8000),@2 tinyint,@3 varchar(8000))SELECT [NUMEROD]+'|'+[DOCUMENT]+'|'+format([SALDO],'N2') [DESCRIP],[NROUNICO],[NUMEROD],[DOCUMENT],[MONTO],format([FECHAE],'yyyy-MM-dd') [FECHAE] FROM [SAACXP] WHERE [TipoCxP]=@1 AND [SALDO]>@2 AND [CodProv]=@3
GO
...
-- Session: 59 | Start: 2026-03-13 14:34:31.120000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT DBO.FN_ADM_PRICEBASE('196852644438',6369.67,0,1) AS PRECIOB
GO

-- Session: 61 | Start: 2026-03-13 14:34:40.163000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CODPROD LIKE '7598008000472%') OR (SP.DESCRIPALL LIKE '7598008000472%') OR (SP.REFERE LIKE '7598008000472%') OR (SP.EXISTEN LIKE '7598008000472%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO
...
-- Session: 59 | Start: 2026-03-13 14:37:11.247000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT * FROM VW_ADM_TAXINVENT WITH (NOLOCK) WHERE  (ESTAXCOMPRA>0) AND CodProd='7594001102359'
GO

-- Session: 59 | Start: 2026-03-13 14:40:24.703000 | Status: runnable | Cmd: EXECUTE
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
-- Session: 71 | Start: 2026-03-13 14:46:00.807000 | Status: running | Cmd: UPDATE
UPDATE SAPROD
SET Refere=b.precio$
from SAPROD as a
inner join CUSTOM_COSTO_COMPRAS as b on (a.CodProd=b.codprod)
...
-- Session: 59 | Start: 2026-03-13 14:53:08.533000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.DEsVence,P.Descto,
     P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.CantEmpaq,P.CostPro,P.Descrip,P.Descrip2,
...
-- Session: 59 | Start: 2026-03-13 15:00:13.717000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.DEsVence,P.Descto,
     P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.CantEmpaq,P.CostPro,P.Descrip,P.Descrip2,
  ...
  FROM VW_ADM_PRODUCTOS P WITH (NOLOCK) 
       ...
       P.CODPROD=C.CODPROD 
 WHERE (P.Activo=1) AND
       ((P.CodProd='7591818000182') OR         (C.CodAlte='7591818000182') OR 
        (P.Refere ='7591818000182'))
GO

-- Session: 59 | Start: 2026-03-13 15:00:40.027000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.EXISTEN ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
  ...
  WHERE ((SP.CODPROD LIKE 'DEXA%') OR (SP.DESCRIPALL LIKE 'DEXA%') OR (SP.REFERE LIKE 'DEXA%') OR (SP.EXISTEN LIKE 'DEXA%')) AND  (SP.Activo=1))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO

-- Session: 59 | Start: 2026-03-13 15:01:00.153000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 *, ISNULL((SELECT COUNT(CODALTE) FROM SAEQUI WITH (NOLOCK)  WHERE CODPROD='AMP_DEXAME_4MG'),0) AS CANTEQUIV,  0 AS ISADIC FROM VW_ADM_PRODUCTOS with (nolock) WHERE (CodProd='AMP_DEXAME_4MG') And (Activo=1)
GO
...
-- Session: 61 | Start: 2026-03-13 15:01:02.197000 | Status: runnable | Cmd: SELECT (STATMAN)
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='6972718560074' OR P.CodProd='6972718560074')
GO

-- Session: 54 | Start: 2026-03-13 15:03:00.100000 | Status: runnable | Cmd: UPDATE
UPDATE SAPROD 
SET PrecioI1=b.precio$1,PrecioI2=b.precio$2,PrecioI3=b.precio$3
from SAPROD as a
inner join CUSTOM_PRECIO_EN_DOLAR as b on (a.CodProd=b.codprod)
...
-- Session: 61 | Start: 2026-03-13 15:03:37.873000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CodProd LIKE 'TINTURA%') OR (SP.DescripAll LIKE 'TINTURA%') OR (SP.Refere LIKE 'TINTURA%') OR (SP.Existen LIKE 'TINTURA%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO

-- Session: 61 | Start: 2026-03-13 15:04:49.410000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7594001101406' OR P.CodProd='7594001101406')
GO

-- Session: 59 | Start: 2026-03-13 15:06:34.637000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT * FROM VW_ADM_TAXINVENT WITH (NOLOCK)  WHERE (CodProd='7591818116005') AND ESTAXCOMPRA=1
GO

-- Session: 59 | Start: 2026-03-13 15:06:35.027000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
-- Session: 64 | Start: 2026-03-13 15:10:29.040000 | Status: running | Cmd: SET COMMAND
SET DATEFORMAT YMD;
Select * from SAFLDREF Where Tabla='SACLIE' Order by NCampo
GO
...
-- Session: 71 | Start: 2026-03-13 15:16:01.527000 | Status: suspended | Cmd: UPDATE
CREATE PROCEDURE [dbo].[sp_sqlagent_set_jobstep_completion_state]
    @job_id                UNIQUEIDENTIFIER,
    @step_id               INT,
    @last_run_outcome      INT,
...
-- Session: 61 | Start: 2026-03-13 15:22:19.857000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT lo.nrounico, lo.nrolote, lo.codubic, dp.descrip, lo.costo, Precio3 AS PRECIO, dbo.FN_ADM_TAXPRODUCT(CodProd,Precio3,1,0,0)+Precio3 As PrecioTx,PrecioU3 AS PRECIOU, dbo.FN_ADM_TAXPRODUCT(CodProd,PrecioU3,1,1,0)+PrecioU3 As PrecioUTx,lo.cantidad, lo.cantidadu, lo.fechae, lo.fechav from SALOTE lo Inner Join sadepo dp on lo.codubic=dp.codubic where (lo.codprod='7592430000185') And (lo.CodUbic='AMR001')  And ((lo.Cantidad>0) Or (lo.CantidadU>0)) Order By lo.codubic, lo.fechav
GO

-- Session: 61 | Start: 2026-03-13 15:22:27.887000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CodProd LIKE 'PROGES%') OR (SP.DescripAll LIKE 'PROGES%') OR (SP.Refere LIKE 'PROGES%') OR (SP.Existen LIKE 'PROGES%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO

-- Session: 70 | Start: 2026-03-13 15:24:49.160000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT  * , ROW_NUMBER() OVER (ORDER BY Descrip ASC) AS ROWNUM   FROM SACLIE WITH (NOLOCK) 
  WHERE (ACTIVO>0))
...
-- Session: 59 | Start: 2026-03-13 15:28:30.763000 | Status: runnable | Cmd: UPDATE
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
-- Session: 66 | Start: 2026-03-13 15:29:38.537000 | Status: running | Cmd: SELECT
-- Query for 'Lotes' worksheet: filters lots based on entry date, rotation and quantity.
SELECT
    SALOTE.CodProd AS Cod,
    SALOTE.NroLote,
...
-- Session: 67 | Start: 2026-03-13 15:29:58.113000 | Status: running | Cmd: SELECT
(@P1 nvarchar(4),@P2 nvarchar(4),@P3 nvarchar(4),@P4 nvarchar(4))
            SELECT
              SACOMP.FechaI,
              SACOMP.FechaE,
...
-- Session: 64 | Start: 2026-03-13 15:38:00.123000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
 INNER JOIN SAEXIS EX ON (EX.CodSucu='00000') And (EX.CodProd=SP.CodProd) And (EX.CodUbic='AMR001')
...
-- Session: 61 | Start: 2026-03-13 15:39:55.420000 | Status: running | Cmd: UPDATE
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
-- Session: 64 | Start: 2026-03-13 15:40:28.353000 | Status: running | Cmd: INSERT
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
-- Session: 61 | Start: 2026-03-13 15:45:22.897000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CodProd LIKE '7591585116000%') OR (SP.DescripAll LIKE '7591585116000%') OR (SP.Refere LIKE '7591585116000%') OR (SP.Existen LIKE '7591585116000%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO

-- Session: 71 | Start: 2026-03-13 15:45:44.010000 | Status: runnable | Cmd: SELECT
(@P1 nvarchar(4),@P2 nvarchar(4),@P3 nvarchar(4),@P4 nvarchar(4))
            SELECT
              SACOMP.FechaI,
              SACOMP.FechaE,
...
-- Session: 61 | Start: 2026-03-13 15:47:20.743000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7591585116000' OR P.CodProd='7591585116000')
GO

-- Session: 61 | Start: 2026-03-13 15:47:50.927000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7596139000026' OR P.CodProd='7596139000026')
GO

-- Session: 64 | Start: 2026-03-13 15:49:40.370000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
 INNER JOIN SAEXIS EX ON (EX.CodSucu='00000') And (EX.CodProd=SP.CodProd) And (EX.CodUbic='AMR001')
...
-- Session: 61 | Start: 2026-03-13 15:50:06.710000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7591818000182' OR P.CodProd='7591818000182')
GO

-- Session: 61 | Start: 2026-03-13 15:50:06.990000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
...
-- Session: 61 | Start: 2026-03-13 15:53:26.427000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 61 | Start: 2026-03-13 15:57:21.617000 | Status: runnable | Cmd: SELECT
SELECT SAFACT.AutSRI, SAFACT.Cambio, 
       SAFACT.CancelA, SAFACT.CancelC, 
       SAFACT.CancelE, SAFACT.CancelG, 
       SAFACT.CancelI, SAFACT.CancelP, 
...
-- Session: 70 | Start: 2026-03-13 15:57:49.420000 | Status: runnable | Cmd: SELECT
SELECT SAFACT.AutSRI, SAFACT.Cambio, 
       SAFACT.CancelA, SAFACT.CancelC, 
       SAFACT.CancelE, SAFACT.CancelG, 
       SAFACT.CancelI, SAFACT.CancelP, 
...
-- Session: 65 | Start: 2026-03-13 16:10:21.787000 | Status: suspended | Cmd: UPDATE
CREATE PROCEDURE sp_sqlagent_log_jobhistory
  @job_id               UNIQUEIDENTIFIER,
  @step_id              INT,
  @sql_message_id       INT = 0,
...
-- Session: 61 | Start: 2026-03-13 16:19:29.920000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7592432010212' OR P.CodProd='7592432010212')
GO
...
-- Session: 64 | Start: 2026-03-13 16:19:54.047000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SAEXIS EX     ON (EX.CODSUCU='00000') AND        (EX.CODPROD=P.CODPROD) AND        (EX.CODUBIC='AMR001') INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='JER_5CC' OR P.CodProd='JER_5CC')
GO

-- Session: 62 | Start: 2026-03-13 16:21:44.253000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CODPROD LIKE 'ALIVET%') OR (SP.DESCRIPALL LIKE 'ALIVET%') OR (SP.REFERE LIKE 'ALIVET%') OR (SP.EXISTEN LIKE 'ALIVET%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 35
GO

-- Session: 65 | Start: 2026-03-13 16:22:37.080000 | Status: running | Cmd: UPDATE
(@P1 float,@P2 nvarchar(12))UPDATE EnterpriseAdmin_AMC.dbo.SAACXP
                   SET Monto = @P1
                   WHERE NumeroD = @P2
GO

-- Session: 54 | Start: 2026-03-13 16:27:26.643000 | Status: running | Cmd: SELECT
(@P1 nvarchar(4),@P2 nvarchar(4),@P3 nvarchar(4),@P4 nvarchar(4))
            SELECT
              SACOMP.FechaI,
              SACOMP.FechaE,
...
-- Session: 64 | Start: 2026-03-13 16:27:39.380000 | Status: running | Cmd: SET COMMAND
SET DATEFORMAT YMD;
SELECT P.*, I.DEsComi AS ITIENECOMI FROM SAPROD P, SAINSTA I WITH (NOLOCK) WHERE P.CodInst=I.CodInst AND (CodProd='AMP_MELOXI')
GO

-- Session: 60 | Start: 2026-03-13 16:27:56.423000 | Status: running | Cmd: SELECT
(@P1 nvarchar(4),@P2 nvarchar(4),@P3 nvarchar(4),@P4 nvarchar(4))
            SELECT
              SACOMP.FechaI,
              SACOMP.FechaE,
...
-- Session: 70 | Start: 2026-03-13 16:38:44.470000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CodProd LIKE 'COLAGEN%') OR (SP.DescripAll LIKE 'COLAGEN%') OR (SP.Refere LIKE 'COLAGEN%') OR (SP.Existen LIKE 'COLAGEN%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 30
GO
...
-- Session: 61 | Start: 2026-03-13 16:39:08.597000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT lo.nrounico, lo.nrolote, lo.codubic, dp.descrip, lo.costo, Precio3 AS PRECIO, dbo.FN_ADM_TAXPRODUCT(CodProd,Precio3,1,0,0)+Precio3 As PrecioTx,PrecioU3 AS PRECIOU, dbo.FN_ADM_TAXPRODUCT(CodProd,PrecioU3,1,1,0)+PrecioU3 As PrecioUTx,lo.cantidad, lo.cantidadu, lo.fechae, lo.fechav from SALOTE lo Inner Join sadepo dp on lo.codubic=dp.codubic where (lo.codprod='7594001102359') And (lo.CodUbic='AMR001')  And ((lo.Cantidad>0) Or (lo.CantidadU>0)) Order By lo.codubic, lo.fechav
GO

-- Session: 61 | Start: 2026-03-13 16:40:37.057000 | Status: running | Cmd: SELECT
create procedure sys.sp_tableswc
(
    @table_name         nvarchar(384)   = null,
    @table_owner        nvarchar(384)   = null,
...
-- Session: 69 | Start: 2026-03-13 16:45:05.993000 | Status: runnable | Cmd: COMMIT TRANSACTION
CREATE PROCEDURE sp_jobhistory_row_limiter
  @job_id UNIQUEIDENTIFIER
AS
BEGIN
...
-- Session: 61 | Start: 2026-03-13 16:46:25.980000 | Status: runnable | Cmd: EXECUTE
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
EXEC SP_ADM_PROXCORREL '00000','','PrxFact',@NUMEROFAC OUTPUT;
INSERT INTO SAFACT ([CodSucu],[TipoFac],[NumeroD],[EsCorrel],[FechaT],[FechaI],[FechaE],[FechaV],[FromTran],[Signo],[CodClie],[CodEsta],[CodUsua],[CodVend],[CodUbic],[Descrip],[Direc1],[ID3],[Monto],[MtoTotal],[Factor],[MontoMEx],[Contado],[TotalPrd],[TExento],[CancelT])
       VALUES ('00000','A',@NUMEROFAC,@CORRELATIVO,GETDATE(),'2026-03-13 16:46:24.980','2026-03-13 16:46:25.152','2026-03-13 16:46:24.980',1,1,'V1001087','CAJA004','22036825','22036825','AMR001','MARIA','CARACAS','V1001087',1537.44,1537.44,443.25,3.47,1537.44,1537.44,1537.44,1537.44);
SET @NROUNICOFAC=IDENT_CURRENT('SAFACT')
SET @NROUNICOLOT=1055789;
UPDATE SAPROD SET 
       FechaUV='2026-03-13 16:46:25.230'
 WHERE (CodProd='7898563802468');
SELECT @EXISTANT=0, @EXISTANTUND=0;
SELECT @EXISTANT=ISNULL(EXISTEN,0), @EXISTANTUND=ISNULL(EXUNIDAD,0)
  FROM SAEXIS WITH (NOLOCK) 
 WHERE (CODPROD='7898563802468') AND 
       (CODSUCU='00000') AND 
       (CODUBIC='AMR001');
EXEC TR_ADM_UPDATE_EXISTENCIAS '00000','7898563802468','AMR001',-2.00,0,'2026-03-13';
SELECT  @EXISTPRD=ISNULL(E.EXISTEN,0) FROM SAEXIS E WITH (NOLOCK)  WHERE (E.CodSucu='00000') And (E.CodProd='7898563802468') And (E.CodUbic='AMR001')
IF @EXISTPRD<0 SET @NUMERRORS=1000;
SET @NROUNICOLOT=1055789
UPDATE SALOTE SET [Cantidad]=[Cantidad]+-2.00 WHERE NroUnico=@NROUNICOLOT
INSERT INTO SAITEMFAC ([CodSucu],[TipoFac],[NumeroD],[NroLinea],[Signo],[FechaE],[CodItem],[Refere],[CodUbic],[Descrip1],[Cantidad],[CantMayor],[Costo],[Factor],[TotalItem],[Precio],[TipoPVP],[PriceO],[CodVend],[CodUsua],[EsExento],[DEsLote],[NroLote],[NroUnicoL],[ExistAntU],[ExistAnt],[FechaL],[FechaV])
       VALUES ('00000','A',@NUMEROFAC,1,1,'2026-03-13 16:46:25.246','7898563802468','0.971241','AMR001','LABIAL SABOR DE CACAO X3.5 GR PRODUZIR',2.00,1.00,385.06,1.00,1537.444,768.722,3,768.722,'22036825','22036825',1,1,'02',ISNULL(@NROUNICOLOT,0),ISNULL(@EXISTANTUND,0),ISNULL(@EXISTANT,0),'2026-02-03 00:00:00.000','1899-12-29 00:00:00.000');
UPDATE SAFACT SET 
   CostoPrd=770.12   ,CostoSrv=0.00   ,MtoComiVta=0.00   ,MtoComiVtaD=0.00   ,MtoComiCob=0.00   ,MtoComiCobD=0.00  WHERE (CODSUCU='00000') AND (TIPOFAC='A') AND (NUMEROD=@NUMEROFAC);
INSERT INTO SAIPAVTA ([NumeroD],[TipoFac],[CodSucu],[CodTarj],[Descrip],[TipoPag],[Monto],[Factor],[FechaE])
       VALUES (@NUMEROFAC,'A','00000','001','TDD',2,1537.44,1.00,'2026-03-13 16:46:21.000');
UPDATE SACONF SET FECHAUP=GETDATE()  WHERE CODSUCU='00000'
  IF @NUMERRORS>0
  BEGIN
    ROLLBACK;
...
-- Session: 54 | Start: 2026-03-13 16:55:10.240000 | Status: runnable | Cmd: SELECT
...
-- Session: 61 | Start: 2026-03-13 17:01:24.513000 | Status: running | Cmd: SELECT (STATMAN)
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 71 | Start: 2026-03-13 17:06:27.183000 | Status: running | Cmd: SELECT
SELECT * FROM EnterpriseAdmin_AMC.dbo.SACOMP
GO
...
-- Session: 58 | Start: 2026-03-13 17:07:55.610000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CODPROD LIKE '7592616361017%') OR (SP.DESCRIPALL LIKE '7592616361017%') OR (SP.REFERE LIKE '7592616361017%') OR (SP.EXISTEN LIKE '7592616361017%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
...
-- Session: 61 | Start: 2026-03-13 17:08:47.357000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT  * , ROW_NUMBER() OVER (ORDER BY Descrip ASC) AS ROWNUM   FROM SACLIE WITH (NOLOCK) 
...
-- Session: 74 | Start: 2026-03-13 17:15:00.880000 | Status: suspended | Cmd: BACKUP DATABASE
CREATE PROCEDURE [dbo].[BackupEnterpriseAdmin_AMC]
AS
BEGIN
    SET NOCOUNT ON;
...
-- Session: 74 | Start: 2026-03-13 17:16:00.360000 | Status: runnable | Cmd: UPDATE
UPDATE SAPROD
SET Refere=b.precio$
from SAPROD as a
inner join CUSTOM_COSTO_COMPRAS as b on (a.CodProd=b.codprod)
...
-- Session: 51 | Start: 2026-03-13 17:17:38.310000 | Status: suspended | Cmd: SELECT
SELECT 
    SAPROD.Descrip, 
    CASE 
        WHEN SATAXPRD.Monto IS NOT NULL THEN SAPROD.Precio1 * (1 + SATAXPRD.Monto / 100) 
...
-- Session: 54 | Start: 2026-03-13 17:18:26.130000 | Status: runnable | Cmd: SELECT
SELECT 
    CASE 
...
-- Session: 54 | Start: 2026-03-13 17:18:26.647000 | Status: runnable | Cmd: SELECT
...
-- Session: 54 | Start: 2026-03-13 17:18:28.120000 | Status: runnable | Cmd: SELECT
SELECT
...
-- Session: 54 | Start: 2026-03-13 17:18:29.690000 | Status: runnable | Cmd: SELECT
WITH TablesAndViews AS
      (SELECT object_id, 'table' AS object_type FROM sys.tables WITH(nolock) UNION ALL SELECT object_id, 'view' AS object_type FROM sys.views WITH(nolock))
      SELECT
      db_id() as database_id,
      i.type,
      i.is_unique,
      ...
      i.is_disabled,
      i.has_filter,
      COUNT_BIG(1) CountOfIndexes,
      t.object_type
      FROM sys.indexes i WITH(nolock)
      INNER JOIN TablesAndViews t
...
-- Session: 54 | Start: 2026-03-13 17:18:33.650000 | Status: runnable | Cmd: SELECT
SELECT
      db_id() as database_id,
...
-- Session: 61 | Start: 2026-03-13 17:19:10.900000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 61 | Start: 2026-03-13 17:20:20.787000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CodProd LIKE '7593255000145%') OR (SP.DescripAll LIKE '7593255000145%') OR (SP.Refere LIKE '7593255000145%') OR (SP.Existen LIKE '7593255000145%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 24
GO
...
-- Session: 70 | Start: 2026-03-13 17:22:09.857000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.DescripAll ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CodProd LIKE 'DICLOFENA%') OR (SP.DescripAll LIKE 'DICLOFENA%') OR (SP.Refere LIKE 'DICLOFENA%') OR (SP.Existen LIKE 'DICLOFENA%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 30
GO

-- Session: 62 | Start: 2026-03-13 17:22:35.887000 | Status: suspended | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT  * , ROW_NUMBER() OVER (ORDER BY Descrip ASC) AS ROWNUM   FROM SACLIE WITH (NOLOCK) 
  WHERE (ACTIVO>0))
...
-- Session: 62 | Start: 2026-03-13 17:22:47.560000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='AMP_FUROSE_20' OR P.CodProd='AMP_FUROSE_20')
GO

-- Session: 61 | Start: 2026-03-13 17:25:11.317000 | Status: running | Cmd: SELECT
DECLARE
 @CODSUCU VARCHAR(5)='00000',
 @CODESTA VARCHAR(15)='',
 @FIELDNAME VARCHAR(20)='PrxFact';
SELECT PREFIJO, VALUEINT, LENCORREL, DESDE, HASTA
  FROM SACORRELSIS WITH (NOLOCK)
 WHERE (CODSUCU=@CODSUCU)
       AND (FIELDNAME=@FIELDNAME)
       AND ((CODESTA IS NULL AND @CODESTA='') OR (CODESTA=@CODESTA))
GO

-- Session: 61 | Start: 2026-03-13 17:25:13.323000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='JELCO' OR P.CodProd='JELCO')
GO

-- Session: 61 | Start: 2026-03-13 17:25:18.840000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='100006' OR P.CodProd='100006')
GO
...
-- Session: 73 | Start: 2026-03-13 17:25:38.947000 | Status: suspended | Cmd: SELECT
SELECT 
    SAPROD.Descrip, 
    CASE 
        WHEN SATAXPRD.Monto IS NOT NULL THEN SAPROD.Precio1 * (1 + SATAXPRD.Monto / 100) 
...
-- Session: 70 | Start: 2026-03-13 17:27:17.020000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7593090003004' OR P.CodProd='7593090003004')
GO

-- Session: 70 | Start: 2026-03-13 17:27:45.690000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 

  WHERE ((SP.CodProd LIKE 'CANDESA%') OR (SP.DescripAll LIKE 'CANDESA%') OR (SP.Refere LIKE 'CANDESA%') OR (SP.Existen LIKE 'CANDESA%')) AND  (SP.Activo=1) AND (SP.EsEnser=0))
 SELECT *, (SELECT MAX(ROWNUM) FROM myCTE WITH (NOLOCK)) AS TOTALROWS
   FROM myCTE WITH (NOLOCK) 
  WHERE RowNum BETWEEN 1 AND 30
GO
...
-- Session: 60 | Start: 2026-03-13 17:31:06.373000 | Status: runnable | Cmd: SELECT
...
-- Session: 60 | Start: 2026-03-13 17:31:23.960000 | Status: runnable | Cmd: SELECT
(@P1 nvarchar(14),@P2 nvarchar(14),@P3 nvarchar(14),@P4 nvarchar(14))
            SELECT
              SACOMP.FechaI,
              SACOMP.FechaE,
...
-- Session: 51 | Start: 2026-03-13 17:33:26.007000 | Status: running | Cmd: SELECT
...
-- Session: 58 | Start: 2026-03-13 17:38:32.297000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 70 | Start: 2026-03-13 17:42:19.930000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7594001101802' OR P.CodProd='7594001101802')
GO

-- Session: 58 | Start: 2026-03-13 17:43:52.887000 | Status: running | Cmd: INSERT
SET DATEFORMAT YMD;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
DECLARE @ErrMsg nvarchar(4000);
DECLARE 
...
EXEC SP_ADM_PROXCORREL '00000','','PrxFact',@NUMEROFAC OUTPUT;
INSERT INTO SAFACT ([CodSucu],[TipoFac],[NumeroD],[EsCorrel],[FechaT],[FechaI],[FechaE],[FechaV],[FromTran],[Signo],[CodClie],[CodEsta],[CodUsua],[CodVend],[CodUbic],[Descrip],[Direc1],[ID3],[Monto],[MtoTotal],[Factor],[MontoMEx],[Contado],[TotalPrd],[TExento],[CancelT])
       VALUES ('00000','A',@NUMEROFAC,@CORRELATIVO,GETDATE(),'2026-03-13 17:43:53.554','2026-03-13 17:43:53.726','2026-03-13 17:43:53.554',1,1,'V11601398','BK03','V12400678','12400678','AMR001','MABEL AYARI BURGOS DE RIVAS','LOS RUICES','V11601398',531.57,531.57,443.25,1.20,531.57,531.57,531.57,531.57);
SET @NROUNICOFAC=IDENT_CURRENT('SAFACT')
SET @NROUNICOLOT=1056785;
UPDATE SAPROD SET 
       FechaUV='2026-03-13 17:43:53.788'
 WHERE (CodProd='7591196006912');
SELECT @EXISTANT=0, @EXISTANTUND=0;
SELECT @EXISTANT=ISNULL(EXISTEN,0), @EXISTANTUND=ISNULL(EXUNIDAD,0)
  FROM SAEXIS WITH (NOLOCK) 
 WHERE (CODPROD='7591196006912') AND 
       (CODSUCU='00000') AND 
       (CODUBIC='AMR001');
EXEC TR_ADM_UPDATE_EXISTENCIAS '00000','7591196006912','AMR001',-1.00,0,'2026-03-13';
SELECT  @EXISTPRD=ISNULL(E.EXISTEN,0) FROM SAEXIS E WITH (NOLOCK)  WHERE (E.CodSucu='00000') And (E.CodProd='7591196006912') And (E.CodUbic='AMR001')
IF @EXISTPRD<0 SET @NUMERRORS=1000;
SET @NROUNICOLOT=1056785
UPDATE SALOTE SET [Cantidad]=[Cantidad]+-1.00 WHERE NroUnico=@NROUNICOLOT
INSERT INTO SAITEMFAC ([CodSucu],[TipoFac],[NumeroD],[NroLinea],[Signo],[FechaE],[CodItem],[Refere],[CodUbic],[Descrip1],[Cantidad],[CantMayor],[Costo],[Factor],[TotalItem],[Precio],[TipoPVP],[PriceO],[CodVend],[CodUsua],[EsExento],[DEsLote],[NroLote],[NroUnicoL],[ExistAntU],[ExistAnt],[FechaL],[FechaV])
       VALUES ('00000','A',@NUMEROFAC,1,1,'2026-03-13 17:43:53.820','7591196006912','0.479676','AMR001','ACETAMINOFEN 650 MG X10 TABL MEDIGEN',1.00,1.00,207.19,1.00,531.568,531.568,3,531.568,'12400678','V12400678',1,1,'258',ISNULL(@NROUNICOLOT,0),ISNULL(@EXISTANTUND,0),ISNULL(@EXISTANT,0),'2026-03-09 00:00:00.000','1899-12-29 00:00:00.000');
UPDATE SAFACT SET 
   CostoPrd=207.19   ,CostoSrv=0.00   ,MtoComiVta=0.00   ,MtoComiVtaD=0.00   ,MtoComiCob=0.00   ,MtoComiCobD=0.00  WHERE (CODSUCU='00000') AND (TIPOFAC='A') AND (NUMEROD=@NUMEROFAC);
INSERT INTO SAIPAVTA ([NumeroD],[TipoFac],[CodSucu],[CodTarj],[Descrip],[TipoPag],[Monto],[Factor],[FechaE])
       VALUES (@NUMEROFAC,'A','00000','001','TDD',2,531.57,1.00,'2026-03-13 17:43:49.000');
UPDATE SACONF SET FECHAUP=GETDATE()  WHERE CODSUCU='00000'
  IF @NUMERRORS>0
  BEGIN
    ROLLBACK;
...
-- Session: 74 | Start: 2026-03-13 17:45:32.573000 | Status: running | Cmd: SELECT
-- This script extracts inventory, costs, rotation, and expiration classification,
-- ensuring that the next expiration date (ProximaFechaV) is only taken from lots with active stock (Cantidad > 0).
...
-- Session: 67 | Start: 2026-03-13 17:50:00.547000 | Status: suspended | Cmd: UPDATE
CREATE PROCEDURE [dbo].[sp_sqlagent_update_jobactivity_start_execution_date]
    @session_id               INT,
    @job_id                   UNIQUEIDENTIFIER,
    @is_system                TINYINT = 0,
...
-- Session: 58 | Start: 2026-03-13 17:50:41.313000 | Status: runnable | Cmd: SELECT
SELECT SAFACT.AutSRI, SAFACT.Cambio, 
       SAFACT.CancelA, SAFACT.CancelC, 
       SAFACT.CancelE, SAFACT.CancelG, 
       SAFACT.CancelI, SAFACT.CancelP, 
...
-- Session: 61 | Start: 2026-03-13 17:51:41.163000 | Status: running | Cmd: SET COMMAND
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 58 | Start: 2026-03-13 17:59:01.097000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
     P.Descrip,P.Descrip2,P.Descrip3,P.PrecioU,P.PrecioU2,P.PrecioU3,P.Precio1,P.Precio2,
     P.Precio3, 0 AS ISADIC , P.COSTPRO AS COSTO   FROM VW_ADM_PRODUCTOS P WITH (NOLOCK)  INNER JOIN SACODBAR C ON        P.CODPROD=C.CODPROD  WHERE (P.EsEnser=0) AND (P.Activo>0)  AND (P.CODPROD=C.CODPROD) AND (C.CodAlte='7591956110248' OR P.CodProd='7591956110248')
GO

-- Session: 58 | Start: 2026-03-13 17:59:13.640000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 66 | Start: 2026-03-13 18:00:04.510000 | Status: suspended | Cmd: UPDATE
...
-- Session: 60 | Start: 2026-03-13 18:03:00.997000 | Status: running | Cmd: UPDATE
UPDATE SAPROD 
SET PrecioI1=b.precio$1,PrecioI2=b.precio$2,PrecioI3=b.precio$3
from SAPROD as a
inner join CUSTOM_PRECIO_EN_DOLAR as b on (a.CodProd=b.codprod)
...
-- Session: 61 | Start: 2026-03-13 18:07:27.247000 | Status: runnable | Cmd: SELECT
SET DATEFORMAT YMD;
Set DateFormat YMD
;WITH MyCTE AS (SELECT '00000' CODSUCU,SP.CODPROD, SP.DESCRIP, SP.DESCRIPALL, SP.REFERE, SP.EXISTEN, SP.EXUNIDAD,SP.DESCOMP, SP.DESSERI, SP.DESLOTE, SP.ESEXENTO, ROW_NUMBER() OVER (ORDER BY SP.CodProd ASC) AS ROWNUM   FROM VW_ADM_PRODUCTOS SP WITH (NOLOCK) 
...
-- Session: 58 | Start: 2026-03-13 18:07:31.770000 | Status: running | Cmd: SELECT
SET DATEFORMAT YMD;
SELECT TOP 1 P.CodProd,P.CodInst,P.DEsCorrel,P.DigitosC,P.Refere,P.DEsSeri,P.DEsComp,P.EsExento,
     P.ExDecimal,P.Unidad,P.UndEmpaq,P.DEsLote,P.EsPesa,P.Tara,P.Factor,P.DEsVence,
     P.Descto,P.CantEmpaq,P.Peso,P.Volumen,P.UndVol,P.EsEmpaque,P.CostPro,P.DescripAll,
...
-- Session: 61 | Start: 2026-03-13 18:10:00.097000 | Status: runnable | Cmd: SELECT
SELECT SAFACT.NumeroD NumeroD_2, 
       SAFACT.TipoFac TipoFac_2, 
       SAITEMFAC.Cantidad, SAITEMFAC.CantidadU, 
       SAITEMFAC.CantMayor, SAITEMFAC.CodItem, 
